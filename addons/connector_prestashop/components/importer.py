...
class PrestashopImporter(AbstractComponent):
    ...
    def _get_prestashop_data(self):
        ...
        try:
            ...
            PrestaShopWebServiceError = Exception

        try:
            return self.backend_adapter.read(self.prestashop_id)
        except PrestaShopWebServiceError as e:
            # Handle deleted products or orphaned references gracefully
            if 'HTTP response is empty' in str(e) or 'Not Found' in str(e):
                _logger.warning(
                    "Skipping %s ID %s (deleted or orphaned reference): %s",
                    self.backend_adapter._prestashop_model,
                    self.prestashop_id,
                    str(e),
                )
                return None
    ...
    def do_in_new_connector_env(self, model_name=None):
        ...
        """
        # Environment.manage() was removed in Odoo 18.0
        # We now directly acquire a cursor from the registry
        ...
        with closing(registry.cursor()) as cr:
            try:
                ...
                # Odoo 18.0: flush() method changed to flush_all()
                new_env.flush_all()
    ...
    def run(self, prestashop_id, **kwargs):
        ...
        # Skip if resource was deleted or doesn't exist (orphaned reference)
        if self.prestashop_record is None:
            _logger.info(
                "Skipping import of %s ID %s (resource not found in PrestaShop)",
                self.model._name,
                self.prestashop_id,
            )
            return
...
class BatchImporter(AbstractComponent):
    ...
    def _run_page(self, filters, **kwargs):
        # Build URL to show what's being fetched
        filter_params = "&".join([f"{k}={v}" for k, v in filters.items()])
        _logger.info(
            "Batch search: GET %s/api/%s?%s",
        ...
        _logger.info(
            ...
            len(record_ids),
            self.backend_adapter._prestashop_model,
            record_ids if len(record_ids) <= 10 else f"{record_ids[:10]}... (showing first 10)"
        )
...
class TranslatableRecordImporter(AbstractComponent):
    ...
    def find_each_language(self, record):
        ...
        for field in self._translatable_fields[self.model._name]:
            # Skip fields that don't exist in the PrestaShop record
            # (e.g., meta_keywords removed in newer PrestaShop versions)
