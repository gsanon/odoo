...
class PartnerImportMapper(Component):
    ...
    def odoo_id(self, record):
        """Match to existing partner by email if matching is enabled.

        This method only applies on record creation (via @only_create decorator).
        It checks if matching is enabled in the backend configuration, then
        searches for an existing partner with the same email address.
        """
        ...
        # Search for existing partners (case-insensitive, parent only)
        ...
        if len(partners) > 1:
            _logger.warning(
                "Multiple partners found with email %s, skipping auto-match", email
            )
            return {}

        if partners:
            partner = partners[0]
            # Check if already bound to another PrestaShop customer
            existing = self.env["prestashop.res.partner"].search(
                [
                    ("odoo_id", "=", partner.id),
            ...
            if existing:
                _logger.info("Partner %s already bound, skipping", partner.id)
                return {}

            _logger.info(
                "Matching PS customer %s to partner %s (email: %s)",
                record.get("id"),
                partner.id,
                email,
            )
