...
# NothingToDoJob was removed in queue_job for Odoo 18.0
# Define it locally to maintain backward compatibility
class NothingToDoJob(Exception):
    """Exception to signal that a job should be skipped."""
    pass


def play_onchanges(model, vals, onchange_fields):
    """Simulate onchanges for Odoo 18.0.

    In Odoo 18.0, onchange_helper is deprecated. This function provides
    a replacement using Model.new() and onchange() to trigger computed fields.

    Args:
        model: The model recordset (e.g., self.env["sale.order"])
        vals: Dictionary of field values
        onchange_fields: List of fields to trigger onchanges for

    Returns:
        Updated dictionary with values from onchange triggers
    """
    # Get onchange specifications from the model
    onchange_specs = model._onchange_spec()

    # Build complete values dict with all model fields
    all_values = {k: v for k, v in vals.items() if k in model._fields}
    not_model_values = {k: v for k, v in vals.items() if k not in model._fields}

    # Initialize missing fields to False (required for onchange to work)
    for field in model._fields:
        if field not in all_values:
            all_values[field] = False

    # Create a new record in memory with the provided values
    new_record = model.new(all_values)

    # Trigger onchanges for each specified field and collect new values
    new_values = {}
    for field in onchange_fields:
        if field in model._fields:
            onchange_result = new_record.onchange(all_values, field, onchange_specs)
            # Extract new values from onchange result
            for fieldname, value in onchange_result.get("value", {}).items():
                if fieldname not in vals:  # Only add truly new values
                    if fieldname in model._fields:
                        field_def = model._fields[fieldname]
                        if field_def.type == "many2one" and isinstance(value, (list, tuple)):
                            value = value[0] if value else False
                        new_values[fieldname] = value
            all_values.update(new_values)

    # Add back non-model values
    for fld, val in not_model_values.items():
        if fld not in all_values:
            all_values[fld] = val

    # Return only original vals + new values from onchanges
    result = {f: v for f, v in all_values.items() if f in vals or f in new_values}
    return result
...
class SaleOrderImportMapper(Component):
    ...
    def _get_sale_order_lines(self, record):
        order_rows = record["associations"].get("order_rows", {})
        _logger.info(
            "PS Order %s - order_rows type: %s, content: %s",
            record.get("id"),
            type(order_rows).__name__,
            order_rows,
        )
        # PrestaShop 9+ returns order_rows as a direct array
        # Older versions return a dict with nested key (order_row or order_rows)
        if isinstance(order_rows, list):
            # PS 9+: direct array
            orders = order_rows
        else:
            # PS 1.6/1.7: nested structure
            orders = order_rows.get(
                self.backend_record.get_version_ps_key("order_row"), []
            )
        if isinstance(orders, dict):
            orders = [orders]
        _logger.info(
            "PS Order %s - returning %d order lines: %s",
            record.get("id"),
            len(orders),
            orders,
        )
    ...
    def _map_child(self, map_record, from_attr, to_attr, model_name):
        ...
        _logger.info(
            "_map_child called for %s, got %d child records: %s",
            model_name,
            len(child_records) if child_records else 0,
            child_records,
        )
        ...
        for child_record in child_records:
            ...
            _logger.info(
                "Reading order detail ID %s via adapter %s",
                child_record.get("id"),
                adapter,
            )
            detail_record = adapter.read(child_record["id"])
            _logger.info("Got detail_record: %s", detail_record)
            ...
            _logger.info("Mapped items: %s", items)
            children.extend(items)
        _logger.info("_map_child returning %d children", len(children))
        return children
    ...
    def finalize(self, map_record, values):
        ...
        # Odoo 18.0: use local play_onchanges helper instead of model method
        sale_vals = play_onchanges(
            self.env["sale.order"],
            sale_vals,
            [
                "fiscal_position_id",
        ...
        for line_vals_command in values["prestashop_order_line_ids"]:
            ...
            # Odoo 18.0: use local play_onchanges helper instead of model method
            line_vals = play_onchanges(
                self.env["sale.order.line"], line_vals, ["product_id"]
            )
...
class SaleOrderImporter(Component):
    ...
    def _add_shipping_line(self, binding):
        ...
        # Odoo 18.0: recompute() removed - computed fields auto-recompute
        # Invalidate cache to ensure fresh values are fetched
