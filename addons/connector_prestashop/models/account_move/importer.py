...
class RefundMapper(Component):
    ...
    def _invoice_line_shipping(self, record, fpos):
        ...
        if not account:
            _logger.warning(
                "No income account found for shipping product %s (ID: %s). "
                "Please configure an income account on the product or its category.",
                product.name,
                product.id,
            )
            # Try to get default account from journal
            journal = self.backend_record.refund_journal_id
            if journal and journal.default_account_id:
                ...
                raise MappingError(
                    _(
                        "No income account found for shipping product '%s'. "
                        "Please configure an income account on the product or its category."
                    )
                    % product.name
                )
    ...
    def _invoice_line(self, record, fpos):
        ...
        # Odoo 18.0 requires account_id to be set
        if not account:
            _logger.warning(
                "No income account found for product %s (ID: %s). "
                "Please configure an income account on the product or its category.",
                product_name,
                product_id,
            )
