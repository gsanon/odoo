...
class ProductCombinationMapper(Component):
    ...
    def barcode(self, record):
        ...
        # Odoo 18.0: check_ean removed, use check_barcode_encoding from odoo.tools.barcode
        if barcode in ["", "0"]:
            backend_adapter = self.component(
                usage="backend.adapter", model_name="prestashop.product.template"
            )
        ...
        if barcode and barcode != "0":
            # Try to validate as EAN-13, but accept other formats too
            try:
                if check_barcode_encoding(barcode, 'ean13'):
                    return {"barcode": barcode}
            except (KeyError, ValueError):
                # Not EAN-13, but still a valid barcode - accept it
                pass
            # Accept any non-empty barcode that's not "0"
            return {"barcode": barcode}
        return {}
