...
class PrestashopBackend(models.Model):
    ...
    _versions = {
        ...
        "8.x": "prestashop.version.key.8.x",
    }

    @classmethod
    def _selection_versions(cls):
        """Available versions
    ...
    state = fields.Selection(selection="_selection_state", string="State", default="draft")
    ...
    matching_customer = fields.Boolean(
        string="Match Customers by Email",
        help="When enabled, imported PrestaShop customers will be matched "
        "to existing Odoo partners by email address. If a matching partner "
        "is found, the import will link to it instead of creating a duplicate.",
    )
    tz = fields.Selection(
        _tz_get,
        "Timezone",
